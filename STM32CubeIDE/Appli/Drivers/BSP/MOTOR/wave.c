//#include "wave.h"
//
//// 假设你在 CubeMX 中启用了 TIM9 来作为新的超声波时钟源
...
//    // 使用 uint16_t 算差值，利用组合语言的补码特性自动处理 65535 到 0 的翻转
...
//    {
//        // 16位定时器下，差值不能超过 65535。这里设定 10000us (10ms) 超时
...
//    // 举例：如果 start_time 是 65530，结束时计数器翻转到了 10
//    // (uint16_t)(10 - 65530) 在数学上等同于 (10 - 65530 + 65536) = 16，溢出问题被硬件完美消除！
...
#define TRIG_PIN  GPIO_PIN_4
#define ECHO_PORT GPIOF
#define ECHO_PIN  GPIO_PIN_7   // 核心改动：Echo 引脚与 MX_TIM9_Init 中的 PF7 保持一致
#define ECHO_TIM_CHANNEL TIM_CHANNEL_1

// 声明 ThreadX 信号量，用于同步中断与测距线程
...
/**
 * @brief  超声波模块初始化
 * @note   需在 ThreadX 启动后的应用初始化（如 tx_application_define）中调用
 */
void Wave_Init(void)
{
...
/**
 * @brief  高精度微秒级阻塞延时（仅用于维持 15us 的 Trig 触发脉冲）
 */
static void delay_us(uint32_t us)
{
    uint16_t start = (uint16_t)__HAL_TIM_GET_COUNTER(&htim9);
...
float Wave_Get_Distance(void)
{
    ...
    // 允许最大等待 25 个 OS Ticks（假设系统 Tick 为 1000Hz，即 25ms 测距超时保护）
    if (tx_semaphore_get(&wave_sem, 3) == TX_SUCCESS)
    {
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    ...
    {
        if (capture_state == 0) // 状态0：成功捕获到 Echo 的上升沿
        {
            // 记录当前计数器的硬件时间戳1
