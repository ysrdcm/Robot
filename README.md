...
- **执行器统一调度**：行驶、蜂鸣器、摄像头舵机、二维云台和激光输出集中由控制线程决策，模式切换和摄像头恢复时先停车并清除旧循迹动作。
- **循迹抗干扰**：同一码型连续出现 `3` 次后才更新动作；全白时按 `HAL_GetTick()` 保持上次动作 `2000 ms`，随后停车。避障后的黑线重捕获使用相同的连续确认规则。
- **遥控失联保护**：人工模式在松开方向键或超过 `500 ms` 未收到有效数据时停车并关闭激光；前方障碍会阻止继续向前和转向，但保留后退脱困能力。
- **视觉稳定与恢复**：人体检测经过连续帧确认和丢失确认，舵机使用死区、低通滤波、限速和短时丢检保持；DCMIPP 取帧超时后由控制线程停止输出并重启采集管线，同时清除旧检测坐标。
- **并发数据保护**：AI 检测状态、显示数据和事件快照分别使用 ThreadX 互斥量保护；ESP-01S 的 UART 中断只写入环形缓冲区，耗时的协议解析和网络发送在线程中完成。
- **网络流量隔离**：ESP-01S 驱动按 `+IPD` 声明长度接收完整请求，将异步 HTTP 数据与同步 AT 应答分流；网页采用手动刷新，图片单独传输，并对短暂 `busy` 状态进行重试。
...
| 循迹码型确认次数 | `3` | `Drivers/BSP/MOTOR/find.c` |
| 全白丢线动作保持 | `2000 ms` | `Drivers/BSP/MOTOR/find.c` |
